...
class OsakaTranslator {
   ...
   * 文を大阪弁に翻訳
   */
  translate(text, articleNum, sentenceIdx) {
  ...
  convertEndings(text) {
    ...
    if (result.includes('なければならない') || result.includes('ねばならない')) {
      ...
      result = result.replace(/([ぁ-ん])なければならない/g, (match, char) => this.conjugateNegative(char) + pattern);
    ...
    // 「〜ない」→「〜へん」「〜あらへん」
    if (result.match(/ない[。、]?$/)) {
      result = result.replace(/([ぁ-ん])ない([。、]?)$/g, (match, char, punctuation) => {
