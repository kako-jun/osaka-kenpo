...
const changedFiles = execSync(
  ...
  { encoding: 'utf-8', cwd: '/home/d131/repos/2025/osaka-kenpo' }
)
  .trim()
  .split('\n')
  .filter((f) => f);
...
for (const filePath of changedFiles) {
  const fullPath = path.join('/home/d131/repos/2025/osaka-kenpo', filePath);
...
if (Object.keys(problematicFiles).length === 0) {
  ...
  for (const law of Object.keys(problematicFiles).sort()) {
    ...
    for (const item of problematicFiles[law]) {
      ...
      console.log(
        `    (2番目の項目の長さ: ${item.secondItemLength}文字, プレビュー: "${item.secondItemPreview}")`
      );
    }
  }
}
