...
export async function loadLawMetadata(
  lawCategory: string,
  lawName: string
): Promise<LawMetadata | null> {
  try {
...
export async function loadFamousArticles(
  lawCategory: string,
  lawName: string
): Promise<FamousArticles | null> {
  try {
    const baseUrl = getBaseUrl();
    const response = await fetch(
      `${baseUrl}/api/metadata/${lawCategory}/${lawName}/famous_articles`
    );
    if (!response.ok) {
      return null;
    }
...
// 個別法律用のバッチメタデータ取得関数
export async function loadLawBatchMetadata(
  lawCategory: string,
  lawName: string
): Promise<{
...
// 個別条文用のバッチデータ取得関数
export async function loadArticleBatchData(
  lawCategory: string,
  lawName: string,
  article: string
): Promise<{
  ...
  try {
    ...
    return {
      ...
      lawMetadata: data.lawMetadata ? LawMetadataSchema.parse(data.lawMetadata) : null,
      allArticles: data.allArticles as any[] | null,
    };
  } catch (error) {
