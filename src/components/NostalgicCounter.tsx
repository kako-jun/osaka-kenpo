...
const NostalgicCounter = ({ 
  counterId, 
  type = 'total',
  digits = '4'
}: NostalgicCounterProps) => {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        // まずインクリメント
        const incrementUrl = `https://nostalgic.llll-ll.com/api/counter?action=increment&id=${encodeURIComponent(counterId)}`;
        await fetch(incrementUrl);
        
        // その後表示用の値を取得（テキスト形式、digits指定付き）
        const displayUrl = `https://nostalgic.llll-ll.com/api/counter?action=display&id=${encodeURIComponent(counterId)}&type=${type}&format=text&digits=${digits}`;
