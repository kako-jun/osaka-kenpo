...
### 0.1. 定番ソースの確認（Wikisourceを優先・存在しない場合は自動フォールバックしない）

歴史法・外国法の原文取得は、毎回検索して最初に見つかったサイトを使うのではなく、確立した定番ソースから確認する（osaka-kenpo#69・taiho_ritsuryo着手時の教訓）。

- **jp_hist（日本の歴史法）は `ja.wikisource.org` が定番ソース**。まずここに全文転記ページがあるか確認する（十七条憲法・武家諸法度・禁中並公家諸法度・御成敗式目・大日本帝国憲法で実績あり）
- **world_hist（外国の歴史法）は `ja.wikisource.org`（日本語訳）と `en.wikisource.org`（原語/英訳）の両方を確認する**（ハンムラビ法典で実績あり。日本語訳のみ・英語訳のみだと版差に気づけないことがある）
- **「あるはず」で終わらせず実際に `curl -s <URL>` で取得し、HTTP 200＋本文が空でないことを確認する**（検索結果のスニペットや記憶だけで存在を仮定しない）
- 取得できたら、条文マーカー（`第N条` / 行頭単独の `一` 等）を機械カウントし、想定条文数（Wikipedia等の通説）と一致するか確認する。欠番がある場合は「転記の欠落」か「史実として欠番（例: ハンムラビ法典§66-99は石碑破損箇所）」かを一次資料の記述で裏取りする
- **Wikisourceに全文転記ページが存在しない場合、個人サイトの現代語訳やブログ等へ安易にフォールバックしない**。国立国会図書館デジタルコレクション・国立公文書館デジタルアーカイブ等の公的機関の資料（画像スキャンでも可）を次点候補として提示し、原文がほぼ現存しない・翻刻テキストがなく人手transcriptionが必要等の制約を明記した上で、**着手方針をユーザーに確認する**（taiho_ritsuryoが該当例: 原文はほぼ散逸し、Web上で読める条文は事実上すべて後代の養老律令由来）
- 複数の訳・転記で条文番号や条数にズレがある場合（例: ハンムラビ法典で日本語訳と英語標準訳で§13の付番が異なる）、自動選択せず選択肢としてユーザーに確認する（§0.2の版確認と同じ扱い）

### 0.2. 複数版（改訂）がある歴史法の版確認（重要）
...
### 0.3. 歴史法の原文取得はWebFetchでなくcurl+パース推奨（重要）
...
- 一次資料（原文）に加えて、可能なら出典付きの読み下し文・現代語訳（Wikipediaの脚注引用等）を別途取得し、意味の面でクロスチェックしてから書き下し文を作成する（版違いの二次資料を誤って使わないよう §0.2 とあわせて注意）
- 複数版が存在する法律を追加した場合、`law_metadata.yaml` の `description` にどの版を採用したか・他にどんな版があるかを明記する（読者が「これは何年のどの版か」を本文だけで把握できるようにする。例: buke_shohatto の description 末尾を参照）
